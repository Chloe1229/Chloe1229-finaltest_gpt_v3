...
if st.session_state.step == 1:
    st.markdown("## Step 1")
    st.write(
        "제6조제1항에 따라 국제공통기술문서(CTD)로 작성하여 허가를 받거나 신고한 의약품의 제조원 또는 제조방법을 변경하는 경우에 해당한다."
    )
...
if st.session_state.step == 2:
    st.markdown("## Step 2")
    st.write(
        "제조에 관한 항목 (CTD 제3부 품질평가 자료 중 3.2.S.2, 3.2.S.3 및 3.2.P.2, 3.2.P.3, 3.2.P.4, 3.2.P.7)을 변경 하는 경우에 해당한다."
    )
...
if st.session_state.step == 3:
    st.markdown("## Step 3")
    st.write(
        "품목의 허가(신고) 사항 중 제조방법에 해당하는 자료(CTD 제3부 품질평가 자료 중 3.2.S.2, 3.2.S.3 및 3.2.P.2, 3.2.P.3, 3.2.P.4, 3.2.P.7)를 국제공통기술문서(CTD)로서 제출하여 심사받은 ‘제조방법 CTD 적용(또는 전환)’ 품목에 해당한다."
    )
...
(근거 : 「의약품의 품목허가·신고·심사 규정」[별표 19])"""
        ...
        st.warning(
            """먼저, CTD 제3부 품질평가 자료 중 3.2.S.2, 3.2.S.3 및 3.2.P.2, 3.2.P.3, 3.2.P.4, 3.2.P.7를 제출하여 제조방법 자료로서 심사 받으시기 바랍니다.  
(근거 : 「의약품의 품목허가·신고·심사 규정」[별표 19])"""
        )
...
if st.session_state.step == 4:
    ...
    st.session_state.step4_selections["s1"] = st.radio(
        "3.2.S.1 일반정보", ["변경 있음", "변경 없음"], key="step4_radio_s1"
    )
    st.session_state.step4_selections["s2"] = st.radio(
        "3.2.S.2 제조", ["변경 있음", "변경 없음"], key="step4_radio_s2"
    )
    ...
    st.session_state.step4_selections["p3"] = st.radio(
        "3.2.P.3 제조", ["변경 있음", "변경 없음"], key="step4_radio_p3"
    )
    st.session_state.step4_selections["p4"] = st.radio(
        "3.2.P.4 첨가제의 관리", ["변경 있음", "변경 없음"], key="step4_radio_p4"
    )
    st.session_state.step4_selections["p7"] = st.radio(
        "3.2.P.7 용기-마개 시스템", ["변경 있음", "변경 없음"], key="step4_radio_p7"
    )

    st.markdown("#### 디자인스페이스")
    st.session_state.step4_selections["ds"] = st.radio(
        "디자인스페이스(Design Space)", ["변경 있음", "변경 없음"], key="step4_radio_ds"
    )
...
# ===== Step5 항목 정의 (전체 하드코딩) =====
step5_items = {
    "s1": {"title": "3.2.S.1 일반정보", "items": {"1": "1. 원료의약품 명칭변경"}},
    "s2": {
        "title": "3.2.S.2 제조",
        "items": {
    ...
    "ds": {
        ...
        "items": {"24": "24. 디자인스페이스(Design Space) 변경"},
    },
}
...
step6_items = {
    "s1_1": {
        "title": "3.2.S.1 일반정보\n1. 원료의약품 명칭변경",
        "subitems": {},
        "requirements": {"1": "1. 유효성분은 그대로 유지된다."},
    },
    "s2_2": {
    ...
    "p4_17": {
        ...
        "requirements": {"1": "1. 첨가제와 완제의약품의 규격에는 변경이 없다."},
    },
    "p4_18": {
...
if st.session_state.step == 6:
    st.markdown("## Step 6")
    st.write(
        "Step 6. Step5에서 '변경 있음'으로 선택된 항목에 대해 충족요건을 모두 선택하세요."
    )
        ...
        if block:
            ...
            for sub_key, sub_text in block.get("subitems", {}).items():
                ...
                if current_key == "p3_15":
                    ...
                    st.radio(
                        sub_text, ["변경 있음"], index=0, key=full_key, disabled=True
                    )
                    ...
                    current_value = st.session_state.step6_selections.get(
                        full_key, "변경 없음"
                    )
...
if st.session_state.step == 7:
    ...
    # title 줄바꿈 처리: \n 또는 \\n → <br> 변환
    st.markdown(
        step6_items[current_key]["title"].replace("\\n", "<br>").replace("\n", "<br>"),
    ...
    for idx in STEP7_GROUPS.get(current_key, []):
        ...
        try:
            cond = eval(
                expr, {}, {"step6_selections": st.session_state.step6_selections}
            )
        except Exception:
            cond = False
...
if st.session_state.step == 8:
    ...
    file_path = file_bytes = html_b64 = None
    ...
    if current_idx is not None:
        ...
        outputs_present = bool(result.get("output_1_tag")) and bool(
            result.get("output_2_text")
        )
